...
// end include: /tmp/tmp94q16p9c.js
...
var ASM_CONSTS = {
 ...
 292222: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 292386: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 292490: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 292661: ($0, $1, $2, $3, $4, $5) => { var deviceType = $0; var channels = $1; var sampleRate = $2; var bufferSize = $3; var pIntermediaryBuffer = $4; var pDevice = $5; if (typeof(window.miniaudio) === 'undefined') { return -1; } var device = {}; var audioContextOptions = {}; if (deviceType == window.miniaudio.device_type.playback && sampleRate != 0) { audioContextOptions.sampleRate = sampleRate; } device.webaudio = new (window.AudioContext || window.webkitAudioContext)(audioContextOptions); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; var channelCountIn = 0; var channelCountOut = channels; if (deviceType != window.miniaudio.device_type.playback) { channelCountIn = channels; } device.scriptNode = device.webaudio.createScriptProcessor(bufferSize, channelCountIn, channelCountOut); device.scriptNode.onaudioprocess = function(e) { if (device.intermediaryBufferView == null || device.intermediaryBufferView.length == 0) { device.intermediaryBufferView = new Float32Array(Module.HEAPF32.buffer, pIntermediaryBuffer, bufferSize * channels); } if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { for (var iChannel = 0; iChannel < channels; iChannel += 1) { var inputBuffer = e.inputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { intermediaryBuffer[iFrame*channels + iChannel] = inputBuffer[iFrame]; } } _ma_device_process_pcm_frames_capture__webaudio(pDevice, bufferSize, pIntermediaryBuffer); } if (deviceType == miniaudio.device_type.playback || deviceType == miniaudio.device_type.duplex) { _ma_device_process_pcm_frames_playback__webaudio(pDevice, bufferSize, pIntermediaryBuffer); for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { var outputBuffer = e.outputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { outputBuffer[iFrame] = intermediaryBuffer[iFrame*channels + iChannel]; } } } else { for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { e.outputBuffer.getChannelData(iChannel).fill(0.0); } } }; if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { navigator.mediaDevices.getUserMedia({audio:true, video:false}) .then(function(stream) { device.streamNode = device.webaudio.createMediaStreamSource(stream); device.streamNode.connect(device.scriptNode); device.scriptNode.connect(device.webaudio.destination); }) .catch(function(error) { console.log("Failed to get user media: " + error); }); } if (deviceType == miniaudio.device_type.playback) { device.scriptNode.connect(device.webaudio.destination); } device.pDevice = pDevice; return miniaudio.track_device(device); },  
 295489: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 295555: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 295948: ($0) => { miniaudio.untrack_device_by_index($0); },  
 295991: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 296116: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; },  
 296242: () => { if (document.fullscreenElement) return 1; },  
 296288: () => { return document.getElementById('canvas').width; },  
 296340: () => { return parseInt(document.getElementById('canvas').style.width); },  
 296408: () => { document.exitFullscreen(); },  
 296435: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 296508: () => { if (document.fullscreenElement) return 1; },  
 296554: () => { return document.getElementById('canvas').width; },  
 296606: () => { return screen.width; },  
 296631: () => { document.exitFullscreen(); },  
 296658: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 296791: () => { return window.innerWidth; },  
 296817: () => { return window.innerHeight; },  
 296844: () => { if (document.fullscreenElement) return 1; },  
 296890: () => { return document.getElementById('canvas').width; },  
 296942: () => { return parseInt(document.getElementById('canvas').style.width); },  
 297010: () => { if (document.fullscreenElement) return 1; },  
 297056: () => { return document.getElementById('canvas').width; },  
 297108: () => { return screen.width; },  
 297133: () => { return window.innerWidth; },  
 297159: () => { return window.innerHeight; },  
 297186: () => { if (document.fullscreenElement) return 1; },  
 297232: () => { return document.getElementById('canvas').width; },  
 297284: () => { return screen.width; },  
 297309: () => { document.exitFullscreen(); },  
 297336: () => { if (document.fullscreenElement) return 1; },  
 297382: () => { return document.getElementById('canvas').width; },  
 297434: () => { return parseInt(document.getElementById('canvas').style.width); },  
 297502: () => { document.exitFullscreen(); },  
 297529: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 297587: () => { return screen.width; },  
 297612: () => { return screen.height; },  
 297638: () => { return window.screenX; },  
 297665: () => { return window.screenY; },  
 297692: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 297745: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 297816: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 297873: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 298129: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 298200: () => { if (document.fullscreenElement) return 1; },  
 298246: () => { return window.innerWidth; },  
 298272: () => { return window.innerHeight; },  
 298299: () => { if (document.pointerLockElement) return 1; }
};
